...
export class MultinomialNaiveBayesTextClassifier {
  ...
  static fromSerialized(payload: NaiveBayesSerialized): MultinomialNaiveBayesTextClassifier {
    ...
    const classifier = new MultinomialNaiveBayesTextClassifier({ smoothing: payload.smoothing });
      ...
      return labels
        .map((label) => {
          ...
          const logProb = Math.log(
            (docCount + this.state.smoothing) / (totalDocs + this.state.smoothing * labels.length),
          );
...
export function trainMultinomialNaiveBayesTextClassifier(
  examples: NaiveBayesExample[],
  options?: { smoothing?: number },
): MultinomialNaiveBayesTextClassifier {
  return new MultinomialNaiveBayesTextClassifier(options).train(examples);
}

export function loadMultinomialNaiveBayesTextClassifier(
  payload: NaiveBayesSerialized,
): MultinomialNaiveBayesTextClassifier {
  return MultinomialNaiveBayesTextClassifier.fromSerialized(payload);
}
