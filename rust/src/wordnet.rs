mod morphology;
use morphology::{legacy_morph_candidate, morph_candidates, normalize_lemma};
...
mod tests {
    ...
    fn candidates_match_basic_inflections() {
        assert!(morph_candidates("parties", "n").contains(&"party".to_string()));
        assert!(morph_candidates("sprinted", "v").contains(&"sprint".to_string()));
        assert!(morph_candidates("faster", "a").contains(&"fast".to_string()));
