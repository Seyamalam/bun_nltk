...
function main() {
  const items: Item[] = [
    ...
    },
    {
      ...
      status: "partial",
      notes: "Fast native/WASM path + trainable model subset, not full upstream model parity.",
    ...
    },
    {
      ...
      status: "partial",
      notes: "Mini + extended bundles plus packed format loader, not full WordNet DB yet.",
    ...
    },
    {
      ...
      status: "partial",
      notes: "CFG + PCFG chart parsing + Earley parsing + lightweight dependency parsing; additional parser families still pending.",
    ...
    },
    {
      ...
      status: "partial",
      notes: "Naive Bayes + MaxEnt + DecisionTree + Logistic + LinearSVM implemented; additional classifier families pending.",
      tests: ["test/classify.test.ts", "test/maxent.test.ts", "test/decision_tree.test.ts", "test/linear_models.test.ts"],
