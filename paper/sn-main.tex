...
\item[$^{a}$] Node column stems via the WASM morphy path (20k words), not Porter; it is a lower bound on comparable work.
\end{tablenotes}
\end{table}
...
\begin{figure}[H]
  ...
  \caption{Left to right: word tokenize, Punkt split, and bigram PMI as input grows 10\,KB $\to$ 10\,MB (log-log); rightmost panel is word tokenize over Brown and Gutenberg. Native and WASM lines stay parallel to Python's across four orders of magnitude of input, i.e.\ the speedup is algorithmic, not a small-input artifact. In the bigram panel the WASM counting path stays flat while Python and the native scoring path grow.}
  \label{fig:scaling}
\end{figure}
...
The scaling behaviour backs the parity claim where it matters. Tokenize speedups hold steady across sizes ($16.4\times$ at 10\,KB, $25.3\times$ at 100\,KB, $25.5\times$ at 1\,MB, $22.9\times$ at 10\,MB), so the advantage does not evaporate on realistic inputs. Punkt holds $22$--$27\times$ throughout. On Brown, tokenize drops from 2.0\,s in Python to 137\,ms native ($14.6\times$); on Gutenberg it is $21.9\times$. The bigram task behaves differently: the WASM counting export finishes 10\,MB in 6.7\,ms flat, while Python takes 1.44\,s ($215\times$) --- but the full PMI top-30 pipeline in native TypeScript costs more than Python at every size because it materializes candidate pairs before scoring, the same weakness visible in Table~\ref{tab:crosstime}. The real-corpus rows also confirm the FreqDist pattern from the fixed-size study: the naive native wrapper loses to Python ($381.9$ vs $254.2$\,ms on Brown) while the counting-only WASM export wins ($86.1$\,ms).
...
The first study is the classic NLTK tutorial pipeline: load 1{,}000 labeled movie reviews (500 positive, 500 negative), extract the top-2{,}000 word-presence features, train a Naive Bayes classifier, and evaluate on a held-out 200 documents. On \emph{bun\_nltk} the whole pipeline (feature extraction, training, evaluation, three fresh predictions) finishes in about 210\,ms and reaches 79.0\,\% holdout accuracy:
