...
NLTK \cite{bird2009nltk,loper2002nltk} defined the teaching API and a lot of us still teach from it. spaCy \cite{spacy2023}, Stanza, and Hugging Face Transformers push accuracy forward, but they do not try to match the NLTK interface. They are not import-compatible and they stay inside Python.

\paragraph{JavaScript NLP.}
\texttt{natural} gives you tokenizers, stemmers (Porter and Snowball), classifiers (Bayes and logistic), and TF-IDF. \texttt{winkNLP} is strong on tokenization and NER. \texttt{compromise} keeps things light for POS and payload tagging. All three are solid within their scope, but the scope is narrow and none of them claims parity. To give a concrete example, \texttt{natural} ports Porter and Bayes but not Punkt, CCG, Glue semantics, tableau provers, or BLEU/GLEU/chrF. What we did is different in kind: we ported the interface, not a model.

\paragraph{Runtimes: Bun, Node, and WebAssembly.}
Bun \cite{bun2024} exposes \texttt{bun:ffi} and \texttt{dlopen} for native libraries and starts up an order of magnitude faster than CPython. WebAssembly \cite{wasm2023} with \texttt{wasm-pack} and \texttt{wasm-bindgen} \cite{wasmPack2024} and Rust \cite{rust2024} lets the same Rust kernels run where \texttt{dlopen} is not available, which is exactly the browser and edge case. Earlier NLP and WASM work tends to accelerate one kernel, a tokenizer for instance. We show you can ship a whole toolkit this way at 92\,KB.

\paragraph{Language resource evaluation.}
LRE cares about how resources get created, annotated, benchmarked, and used, including the software that wraps those resources. A toolkit parity artifact fits there naturally. Reviewers can judge it on coverage, reproducibility, and measured evaluation rather than on whether it proposes a new model.
