...
import { featureEarleyParse, parseFeatureCfgGrammar } from "../index";
...
function main() {
  ...
  const py = JSON.parse(new TextDecoder().decode(proc.stdout).trim()) as { parse_count: number };
  const parity = jsTrees.length === py.parse_count && jsTrees.length > 0 && bad.length === 0;
  if (!parity) {
    throw new Error(`feature earley parity failed: js=${jsTrees.length} py=${py.parse_count} bad=${bad.length}`);
  }
