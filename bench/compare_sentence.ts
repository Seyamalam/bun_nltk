...
function runNative(text: string, rounds: number) {
  ...
  for (let i = 0; i < rounds; i += 1) {
    ...
    sentences = sentenceTokenizeSubset(text);
    timings.push((performance.now() - started) / 1000);
  }

  return {
    sentence_count: sentences.length,
    sample: sentences.slice(0, 50),
...
function runPython(inputPath: string): PythonResult {
  const proc = Bun.spawnSync(["python3", "bench/python_sentence_baseline.py", "--input", inputPath], {
...
function main() {
  ...
  const sampleMatches =
    JSON.stringify(native.sample) === JSON.stringify(python.sentences.slice(0, native.sample.length));
